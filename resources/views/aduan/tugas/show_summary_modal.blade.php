...
<!--                    <td>Wujud Kes</td>
        ...
        <div class="table-responsive">
            <table>
                <tr style="background: #115272; color: white; "><th colspan="8">@lang('public-case.case.transactionsm')</th></tr>
<!--                <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            {{Carbon::parse($sorotan->CD_CREDT)->format('d-m-Y h:i A')}}</td>
        ...
        <div class="table-responsive">
            <table>
                <tr style="background: #115272; color: white;">
                    <th colspan="6">Alasan ( AD51 / AD52 )</th>
                </tr>
                <tr>
                    <th>Bil.</th>
                    <th>Tempoh</th>
                    ...
                    <th>Tarikh Mula</th>
                    <th>Tarikh Tamat</th>
                    <th>Tindakan Oleh / Pada</th>
                </tr>
                @php $i = 1; @endphp
                @foreach ($trnsksi as $sorotan)
                @if($sorotan->CD_REASON_DURATION)
                <tr>
                    <td style="width: 5%">{{ $i++ }}</td>
                    <td>{{ $sorotan->CD_REASON_DURATION ?? '' }}</td>
                    <td>{{ $sorotan->reason ? $sorotan->reason->descr : '' }}</td>
                    <td>
                        {{ $sorotan->CD_REASON_DATE_FROM ? Carbon::parse($sorotan->CD_REASON_DATE_FROM)->format('d-m-Y') : '' }}
                    </td>
                    <td>
                        {{ $sorotan->CD_REASON_DATE_TO ? Carbon::parse($sorotan->CD_REASON_DATE_TO)->format('d-m-Y') : '' }}
                    </td>
                    <td>
                        {{ $sorotan->UserCreate ? $sorotan->UserCreate->name : '' }}
                        {{ $sorotan->CD_CREDT ? Carbon::parse($sorotan->CD_CREDT)->format('(d-m-Y h:i A)') : '' }}
                    </td>
                </tr>
                @endif
                @endforeach
