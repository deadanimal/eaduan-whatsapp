                            ...
                            <div id="countduration"
                                ...
                                <div class="col-lg-7">
                                    <p class="form-control-static">
                                        {{-- $countDuration --}}
                                        {{ $mPenugasan->countDuration ?? 0 }}
                                    </p>
                                </div>
                            </div>
                            <div id="countCurrentDuration"
                                class="form-group"
                                style="display: {{ !empty(old('CA_INVSTS')) ? 'block' : 'none' }} ;">
                                {{ Form::label('', 'Tempoh Seliaan', ['class' => 'col-lg-5 control-label']) }}
                                <div class="col-lg-7">
                                    <p class="form-control-static">
                                        {{-- $countDuration --}}
                                        {{ $mPenugasan->countCurrentDuration ?? 0 }}
                                    </p>
                                    {{-- Form::hidden('CD_REASON_DURATION', $countDuration, ['class' => 'form-control input-sm', 'readonly' => true]) --}}
                                    {{ Form::hidden('CD_REASON_DURATION', $mPenugasan->countCurrentDuration ?? 0, ['class' => 'form-control input-sm', 'readonly' => true]) }}
                                </div>
                            </div>
                            {{-- @if($countDuration >= 4) --}}
                            @if(($mPenugasan->countCurrentDuration) && ($mPenugasan->countCurrentDuration >= 4))
                            <div id="cd_reason"
...
<script type="text/javascript">
    ...
    $('#CA_INVSTS').on('change', function (e) {
        ...
        if(CA_INVSTS === ''){
            $('#div_BLOCK').hide();
            $('#countduration').hide();
            $('#countCurrentDuration').hide();
            $('#cd_reason').hide();
            $('#data_5').hide();
        } else {
            $('#div_BLOCK').show();
            $('#countduration').show();
            $('#countCurrentDuration').show();
            $('#cd_reason').show();
