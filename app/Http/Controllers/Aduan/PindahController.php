...
class PindahController extends Controller {
    ...
    public function edit($CASEID) {
        ...
        $mPindah->countDuration = CaseDetailRepository::calculate($CASEID);
        $mPindah->countCurrentDuration = CaseDetailRepository::calculateCurrentPeriod($CASEID);
...
//        $mPindah = PindahAduan::where(['CA_INVSTS' => 1, 'CA_BRNCD' => Auth::user()->brn_cd])->orderBy('CA_RCVDT', 'DESC');
        ...
        if ($request->mobile) {
            $mPindah = $mPindah->offset($request->offset)->limit($request->count)->get()
                ->each(function ($item, $key) {
                    // $item->countDuration = CaseInfoRepository::getDuration($item->CA_CASEID);
                    // $item->reasonIndicator = $item->countDuration >= 4 ? 1 : 0;
            ...
            return response()->json(['data' => $mPindah
...
//                return '<div style="max-height:80px; overflow:auto">'.$pindahAduan->CA_SUMMARY.'</div>';
                ...
                ->addColumn('tempoh', function(PindahAduan $pindahAduan) use ($TempohPertama,$TempohKedua,$TempohKetiga) {
                    ...
                    // }
                    $totalDuration = CaseDetailRepository::calculate($pindahAduan->CA_CASEID);
                ...
                ->addColumn('countCurrentDuration', function(PindahAduan $pindahAduan) use ($TempohPertama,$TempohKedua,$TempohKetiga) {
                    $totalDuration = CaseDetailRepository::calculateCurrentPeriod($pindahAduan->CA_CASEID);
                    // if ($totalDuration >= 0 && $totalDuration <= $TempohPertama->code)
                    //     return '<div style="background-color:#3F6; color: white;" align="center"><strong>'.$totalDuration.'</strong></div>';
                    // else if ($totalDuration > $TempohPertama->code && $totalDuration <= $TempohKedua->code)
                    //     return '<div style="background-color:#FF3;" align="center"><strong>'.$totalDuration.'</strong></div>';
                ...
                ->addColumn('reason_indicator', function(PindahAduan $pindahAduan) {
                    // $mPindahCaseDetail = PindahAduanDetail::
                    //     where('CD_CASEID', $pindahAduan->CA_CASEID)
                    //     ->where('CD_INVSTS', '0')
                    //     ->orderBy('CD_CREDT', 'desc')
                ...
                ->rawColumns(['check', 'CA_CASEID', 'CA_SUMMARY', 'tempoh', 'action', 'countCurrentDuration'])
...
//            ->rawColumns(['CD_DOCATTCHID_ADMIN'])
    ...
    public function EditKelompok(request $Request) {
        ...
        foreach($arrCASEID as $caseid) {
            ...
            // }
            $countDuration[$caseid] = CaseDetailRepository::calculate($caseid);
            $countCurrentDuration[$caseid] = CaseDetailRepository::calculateCurrentPeriod($caseid);
        }
        $mUser = User::find(Auth::User()->id);
        $mRefState = Ref::where(['cat' => '17', 'status' => '1'])
            ->pluck('descr', 'code');
        $caseReasonTemplates = CaseReasonTemplate::where(['category' => 'AD51', 'status' => '1'])
            ->orderBy('sort', 'asc')
            ->pluck('descr', 'code');
        return view('aduan.pindah.editkelompok', compact('arrCASEID', 'mUser', 'mRefState', 'caseReasonTemplates', 'countDuration', 'countCurrentDuration'));
    }
