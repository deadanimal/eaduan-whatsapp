...
class SiasatController extends Controller
{
    ...
    public function GetDataTable(Request $request)
    {
        ...
        if ($request->mobile) {
            $mSiasat = $mSiasat->offset($request->offset)->limit($request->count)->get()
                ->each(function ($item, $key) {
                    // $item->countDuration = Penyiasatan::GetDuration($item->CA_ASGDT, $item->CA_CASEID);
                    ...
                    // $item->reasonIndicator = $item->countDuration >= 22 ? 1 : 0;
                    $item->reasonIndicator = $item->countCurrentDuration >= 22 ? 1 : 0;
                });
            // return response()->json(['data' => $mSiasat->offset($request->offset)->limit($request->count)->get()->toArray()]);
            return response()->json(['data' => $mSiasat
...
//                    return $Siasat->GetDuration($Siasat->CA_RCVDT, $Siasat->CA_CASEID);
                ...
                $totalDuration = CaseDetailRepository::calculate($Siasat->CA_CASEID);
            ...
            ->addColumn('count_duration', function (Penyiasatan $Siasat) {
                ...
                $count_duration = CaseDetailRepository::calculateCurrentPeriod($Siasat->CA_CASEID);
                return '<div align="center"><strong>' . $count_duration . '</strong></div>';
            })
            ->addColumn('reason_indicator', function (Penyiasatan $Siasat) {
                // $count_duration = $Siasat->GetDuration($Siasat->CA_ASGDT, $Siasat->CA_CASEID);
                $count_duration = CaseDetailRepository::calculateCurrentPeriod($Siasat->CA_CASEID);
                $reasonIndicator = $count_duration >= 22 ? 1 : 0;
    ...
    public function edit($CA_CASEID)
    {
        ...
        if ($mGabungOne) {
            ...
            foreach($mGabungAll as $mergeRow) {
                ...
                $countDurationReceived[$mergeRow->CR_CASEID] = CaseDetailRepository::calculate($mergeRow->CR_CASEID);
                $countDuration[$mergeRow->CR_CASEID] = CaseDetailRepository::calculateCurrentPeriod($mergeRow->CR_CASEID);
                // if($countDurationReceived[$mergeRow->CR_CASEID] >= 22) {
                if($countDuration[$mergeRow->CR_CASEID] >= 22) {
            ...
            $countDurationReceived = CaseDetailRepository::calculate($mSiasat->CA_CASEID);
            $countDuration = CaseDetailRepository::calculateCurrentPeriod($mSiasat->CA_CASEID);
        ...
        if ($request->hantar) {
            if ($request->CA_INVSTS == '3' && $request->CA_SSP == 'YES') {
                ...
                [
                    ...
                    'CA_SSP.required_if' => 'Ruangan Wujud Kes diperlukan.',
            ...
            if(is_array($request->countDurationReceived)) {
                ...
                foreach($arrayCount as $key => $value) {
                    ...
                    if($request->CD_REASON_DURATION[$key] >= 22) {
                ...
                $v->sometimes('CD_REASON', 'required', function ($input) {
                    ...
                    return $input->CD_REASON_DURATION >= 22 && !empty($input->CA_INVSTS);
                });
                $v->sometimes(['CD_REASON_DATE_FROM', 'CD_REASON_DATE_TO'], 'required', function ($input) {
                    ...
                    return $input->CD_REASON_DURATION >= 22 && $input->CD_REASON == 'S2' && !empty($input->CA_INVSTS);
                });
            }
            $v->validate();
        }
