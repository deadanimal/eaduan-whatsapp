...
class IntegritiController extends Controller
{
    ...
    public function StatistikAduanMengikutStatus(Request $request) 
    {
        ...
        if ($request->has('cari') || $request->get('excel') == '1' || $request->get('pdf') == '1') {
            ...
            $ddUri['main'] = "/report/integrity/status/detail";
            $ddUri['param'] = "?datestart=" . $DATE_FROM->format('d-m-Y') . "&dateend=" . $DATE_TO->format('d-m-Y');
            $ddUri['full'] = $ddUri['main'] . $ddUri['param'];

            foreach($statusList as $key => $value) {
                if(!in_array($key, ['010'])) {
                    $ddUri[$key] = $ddUri['full'] . "&status=" . $key;
                }
            }
        }
        ...
        if ($request->has('excel')) {
            ...
            return view('laporan.integriti.statistikaduanmengikutstatus.index', compact('request', 'datas', 'DATE_FROM', 'DATE_TO', 'statusList' ,'mRefStatus', 'ddUri'));
        }
    }
}