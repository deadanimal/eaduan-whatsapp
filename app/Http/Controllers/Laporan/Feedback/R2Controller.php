...
class R2Controller extends Controller
{
    ...
    public function index(Request $request)
    {
		...
		// prepare for sort the data by name
		 array_multisort(array_column($data_final, 'name'), SORT_ASC, $data_final);
            ...
            $name_list = [];
            foreach ($data_final as $key => $datum) {
                array_push($name_list, $data_final[$key]['name']);
            }

            $close_list = [];
            foreach ($data_final as $key => $datum) {
                array_push($close_list, $data_final[$key]['close']);
            }
            
            $ticket_list = [];
            foreach ($data_final as $key => $datum) {
                array_push($ticket_list, $data_final[$key]['ticket']);
            }

            // dd( $data_final, $name_list, $close_list, $ticket_list);

            switch ($gen) {
                case 'p':
                    $this->generatePdf($title, $date_start, $date_end, $data_final, $data_final_total_by_type, $gen, $name_list);
                    break;
                case 'e':
                    $this->generateExcel('csv', $title, $date_start, $date_end, $data_final, $data_final_total_by_type, $name_list);
                    break;
                case 'c':
                    $this->generateExcel('csv', $title, $date_start, $date_end, $data_final, $data_final_total_by_type, $name_list);
                    break;
                ...
                default:
                    return view('laporan.feedback.r2.index',
                        compact('title', 'year_list', 'date_start', 'date_end', 'gen', 'is_search'
                            , 'data_final', 'data_final_total_by_type', 'name_list', 'close_list', 'ticket_list')
                    );
                    break;
            }
        }
     ...
     */
    public function generatePdf($title, $date_start, $date_end, $data_final, $data_final_total_by_type, $gen,$name_list)
    {
        // dd("masuk sini");
        $close_list = [];
        $ticket_list = [];
        $pdf = PDF::loadView('laporan.feedback.r2.pdf', compact('title', 'year_list', 'date_start'
            , 'date_end', 'gen', 'is_search', 'data_final', 'data_final_total_by_type', 'name_list', 'close_list', 'ticket_list'), [], ['format' => 'A4-L']);
     ...
     * @return mixed
     */
    public function generateExcel($type, $title, $date_start, $date_end, $data_final, $data_final_total_by_type, $name_list)
    {
        $close_list = [];
        $ticket_list = [];

        return Excel::create($title . date("_Ymd_His"), function ($excel) use (
            $title, $date_start, $date_end, $data_final, $data_final_total_by_type, $name_list, $close_list, $ticket_list) {
